            ...
            tabPanel("Overview", 
                     ...
                     box(width = NULL, title = "Introduction", status = "primary", fluidRow(column(width = 12,
                                         ...
                                         p('We have developed an omics-based risk calculator that enables the early identification of COVID-19 progression and therefore allows for early intervention. 
                                                                         The model classifies patients into “moderate” or “severe” COVID-19 risk. The layout and features of this application are as follows.'),
                                         tags$div(tags$ul(
                                           tags$li("Data: Separate tabs for each omics platform, with which we hope to provide insights into the features and their relationship with COVID-19 severity."),
                                           tags$li("Design: This tab contains a diagram of our entire workflow and approach."),
                                           tags$li("Prediction: This tab contains the actual risk calculator which adapts to user-provided data and provides predictions about each patients’ COVID-19 severity based on our final models.")), style = "font-size: 15px")
                                         ))
                     )),box(width = NULL, title = "Disclaimer", status = "primary",
                            p("The interpretation of the results of the calculator is not recommend for those without appropriate medical and clinical knowledge. We do not accept any liability, including for any loss or damage, resulting from the reliance on the content, nor do we vouch for its accuracy and completeness.")
                            )
                     ),
                           ...
                           fluidRow(
                             box(width = NULL, title = "Instructions for uploading datasets", status = "primary", fluidRow(column(width = 12,
                                                                                                                                   div(style = "font-size:15px;",
                                                                                                                                       ...
                                                                                                                                       p('The current version of the shiny app only supports datasets with the following properties:'),
                                                                                                                                       tags$div(tags$ul(
                                                                                                                                         tags$li("The clinical data must contain 3 columns with column names: sample_id,sex,age. This dataset is compulsory."),
                                                                                                                                         tags$li("The omics data should contain the top 100 features in the corresponding top tables ranked by moderate t-statistics for each platform"),
                                                                                                                                         tags$li("Note that in order to make a second prediction, a user must first reload the shiny app.")), style = "font-size: 15px"),
                                                                                                                                       p("Example data is provided in the “demo_uploading_files” folder within the repository, which contains two clinical datasets, one from Su et al. and the other from Shen et al., as well as the processed omics data we used from each paper. A small set of the samples from the two datasets are selected respectively, for the purpose of demonstration. The clinical data should be uploaded along with the relevant omics platform data, using as many or as few platforms as wanted, so long as they all come from the same paper. It should be noted that the omics platforms are denoted with p for proteomics, m for metabolomics, and r for transcriptomics in the file names."),
...
server <- function(input, output) {
  ...
  output$ui1 <- renderUI({
    p("To train our models, we used data from the following paper by Su et. al: ",em("Multi-Omics Resolves a Sharp Disease-State Shift between Mild and Moderate COVID-19"), " ( 2020, Cell 183, 1479–1495. DOI: ",
